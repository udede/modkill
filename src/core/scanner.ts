import { readdir, stat } from 'node:fs/promises';
import path from 'node:path';

export interface ModuleInfo {
  path: string;
  sizeBytes: number;
  mtimeMs: number;
  hasPackageJson: boolean;
}

export interface ScanOptions {
  rootPath: string;
  depth?: number;
  excludeGlobs?: string[];
  followSymlinks?: boolean;
}

const DEFAULT_EXCLUDES = [
  '.git',
  '.svn',
  '.hg',
  'Library',
  'Windows',
  'System32',
  'AppData',
];

function shouldExclude(name: string): boolean {
  return DEFAULT_EXCLUDES.includes(name);
}

async function getDirectorySize(dirPath: string): Promise<number> {
  let total = 0;
  const entries = await readdir(dirPath, { withFileTypes: true });
  for (const entry of entries) {
    const full = path.join(dirPath, entry.name);
    try {
      if (entry.isFile()) {
        const s = await stat(full);
        total += s.size;
      } else if (entry.isDirectory()) {
        total += await getDirectorySize(full);
      }
    } catch {
      // Skip unreadable entries
    }
  }
  return total;
}

export class ModuleScanner {
  private readonly maxDepth: number;
  private readonly followSymlinks: boolean;

  constructor() {
    this.maxDepth = 6;
    this.followSymlinks = false;
  }

  async scan(options: ScanOptions): Promise<ModuleInfo[]> {
    const depthLimit = options.depth ?? this.maxDepth;
    const results: ModuleInfo[] = [];
    await this.walk(options.rootPath, 0, depthLimit, results, options.followSymlinks ?? this.followSymlinks);
    return results;
  }

  private async walk(
    currentPath: string,
    currentDepth: number,
    depthLimit: number,
    results: ModuleInfo[],
    followSymlinks: boolean,
  ): Promise<void> {
    if (currentDepth > depthLimit) return;

    let entries;
    try {
      entries = await readdir(currentPath, { withFileTypes: true });
    } catch {
      return;
    }

    for (const entry of entries) {
      const full = path.join(currentPath, entry.name);
      if (shouldExclude(entry.name)) continue;

      if (entry.isSymbolicLink()) {
        if (!followSymlinks) continue;
      }

      if (entry.isDirectory()) {
        if (entry.name === 'node_modules') {
          try {
            const s = await stat(full);
            const sizeBytes = await getDirectorySize(full);
            const hasPackageJson = await this.hasPackageJson(path.dirname(full));
            results.push({ path: full, sizeBytes, mtimeMs: s.mtimeMs, hasPackageJson });
          } catch {
            // ignore
          }
          continue; // do not descend into node_modules children
        }
        await this.walk(full, currentDepth + 1, depthLimit, results, followSymlinks);
      }
    }
  }

  private async hasPackageJson(dirPath: string): Promise<boolean> {
    try {
      await stat(path.join(dirPath, 'package.json'));
      return true;
    } catch {
      return false;
    }
  }
}
